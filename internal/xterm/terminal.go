...
// ScrollLines scrolls the viewport by disp lines (negative = up / history, positive = down).
func (t *Terminal) ScrollLines(disp int) {
	t.bufferService.ScrollLines(disp, false)
}

// ScrollReset resets the scroll position to the bottom (live view).
func (t *Terminal) ScrollReset() {
	buf := t.bufferService.Buffer()
	t.bufferService.IsUserScrolling = false
	buf.YDisp = buf.YBase
	t.bufferService.OnScrollEmitter.Fire(buf.YDisp)
}
